...
export class OperationAchatService {
...
downloadFile(data, filename='data') {
  ...
  let csvData = this.ConvertToCSV(data, ['idOperation','type', 'reqId', 'poNb',
   'montant', 'detail', 'statut', 'attachement', 'dateEnreg', 'userName']);
...
return this.http.get(url+"/listOperationAchats", {headers : headers});
}

uploadPhotoAttachements(url, file: File, idOperation):Observable<HttpEvent<{}>>{
  let formdata : FormData = new FormData();
  formdata.append('file', file);
  let head = new HttpHeaders({'authorization': 'Bearer '+ this.authService.jwt});
  const req = new HttpRequest('POST', url+'/uploadAttachement/'+idOperation, formdata,{
    headers:head,
    reportProgress:true,
    responseType : 'text'
  });
